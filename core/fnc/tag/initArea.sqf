...
for "_i" from 1 to _n do {
    ...
    if !(surfaceIsWater _sel_pos) then {
        ...
        if (!isNull _road && {random 1 > 0.6}) then { // Draw sentences
            private _sentences = (((selectRandom btc_type_tags_sentences)) splitString " ");
            {
                private _word = _x;
                {
            ...
            _sel_pos set [2, 0];
            _array pushBack [_sel_pos, [_v1, _v2], "cvo\img\yankeegohometiled.paa", selectRandom btc_type_tags];
        };
